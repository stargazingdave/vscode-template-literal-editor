...
function activate(_context) {
    ...
    async function activateSubdocument(language, editor, start, end) {
        ...
        await fs_1.promises.writeFile(tempPath, doc.getText(templateRange), 'utf8');
...
function resolveLanguageId(tagOrMarker) {
    if (!tagOrMarker)
        return null;
 ...
 */
function getLeadingCommentMarker(doc, templateStart) {
    ...
    while ((m = blockRe.exec(beforeBacktick)) !== null)
        lastBlock = m[1];
